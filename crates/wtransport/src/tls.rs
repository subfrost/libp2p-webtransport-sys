...
pub mod client {
    ...
    impl ServerCertVerifier for ServerHashVerification {
        fn verify_server_cert(
            ...
            log::debug!("Custom server cert verifier");
            ...
            let x509 = X509Certificate::from_der(end_entity.as_ref())
                .map_err(|e| {
                    log::error!("Cannot parse certificate: {}", e);
            ...
            {
                log::error!("Certificate public key is not P-256");
                return Err(rustls::CertificateError::UnknownIssuer.into());
            }
            ...
            if self.hashes.contains(&end_entity_hash) {
                log::debug!("Certificate hash is trusted");
                Ok(ServerCertVerified::assertion())
            } else {
                log::error!("Certificate hash is not trusted");
